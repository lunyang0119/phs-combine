...
명령어는 목록에 보이는데 실행하면 `Application command '검색' not found` 가 뜨는 경우는, 예전 봇이 서버(길드)에 남겨 둔 명령어 사본이 원인입니다. 봇을 재시작하면 시작 시 길드별로 목록을 맞춰 사본을 지웁니다. 재시작 없이 정리하려면 봇 소유자가 아무 채널에서나 `!동기화` 를 입력하세요.
