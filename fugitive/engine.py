...
def hack_available(st: GameState, hack: str) -> Tuple[bool, str]:
    ...
    if dev and st.game_map.rooms[f.room].device != dev:
        return False, f"{hack} 은(는) {dev} 장치가 있는 방에서만 사용할 수 있습니다 (현재 {st.game_map.rooms[f.room].device})"
...
def submit_fugitive_order(st: GameState, move: Optional[str], hack: Optional[str] = None,
    ...
    if hack is not None:
        ...
        if hack == "doorlock":
            ...
            if rng_ >= 0:
                a, b = target.split("-")
                if min(gmap.distance(f.room, a), gmap.distance(f.room, b)) > rng_:
                    raise RuleError(f"문 잠금 범위 초과 (doorlock_range={rng_})")
        elif hack == "distract":
            if not target or target not in gmap.rooms:
