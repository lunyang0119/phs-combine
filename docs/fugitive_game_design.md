...
- Larger maps are generated by `config._grid_map(cols, rows, blocked, device_offset)`: a full grid with the listed doors bricked up, devices laid as `(col + 2·row + offset) mod 4` so grid neighbours never share a device. `tests/test_engine.py` asserts every built-in map is valid, connected, has a unique door structure, ≥3 rooms per device, and at most ¼ dead-end rooms.
  - `car2077_18` — 6×3 long carriage; the upper and lower corridors connect only through a few staggered doors.
  - `car2077_20` — 5×4; the centre column (B3/C3) is a walled shaft, so the two halves meet via row A, row D and the shaft.
  - `car2077_24` — 6×4 serpentine maze (longest shortest path 9).
  - `car2077_25` — 5×5; C3 is the hub, each corner pocket has a single detour.
